...
public class AppManager implements RampLoad.DownloadListener, RampLoad.StatusListener {
  ...
  private RampLoadDownload createRampLoadDownloadFromAppDetail(AppDetail appDetail) {
    String url = String.format(MyMarketConstants.APP_DOWNLOAD_URL, appDetail.id, appDetail.versionNumber);
    String destination = String.format(MyMarket.getInstance().getAppsDestinationFile(), appDetail.id, appDetail.name.trim().replaceAll("\\s+", ""));
    return new RampLoadDownload(appDetail.id, appDetail.name, url, destination);
  }
  ...
  void update(AppDetail appDetail) {
    String path = String.format(MyMarket.getInstance().getAppsDestinationFile(), appDetail.id, appDetail.name.trim().replaceAll("\\s+", ""));
